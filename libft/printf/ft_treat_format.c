...
void	__treat_format(t_struct *content, va_list argument)
{
	if (content->conversion == 'd' || content->conversion == 'i')
		...
		__base((__uint64_t)va_arg(argument, unsigned int), HEX_MIN, content);
	else if (content->conversion == 'u')
		__base((__uint64_t)va_arg(argument, unsigned int), DEC, content);
	else if (content->conversion == 'X')
		__base((__uint64_t)va_arg(argument, unsigned int), HEX_MAJ, content);
